...
export class WordMasteryService {
  ...
  static async recordAnswer(userId: string, input: WordMasteryInput) {
    const now = new Date();

    const stats = await prisma.wordMasteryStats.upsert({
      where: {
        userId_wordId: {
          userId,
          wordId: input.wordId,
  ...
  static async recalculateMastery(userId: string, wordId: string) {
    const stats = await prisma.wordMasteryStats.findUnique({
      where: {
        userId_wordId: {
          userId,
          wordId,
    ...
    if (stats.lastAnsweredAt) {
      const daysSinceLastAnswer = Math.floor(
        (Date.now() - stats.lastAnsweredAt.getTime()) / (1000 * 60 * 60 * 24)
      );
      if (daysSinceLastAnswer <= 1) recencyScore = 100;
      else if (daysSinceLastAnswer <= 3) recencyScore = 80;
      else if (daysSinceLastAnswer <= 7) recencyScore = 60;
      else if (daysSinceLastAnswer <= 14) recencyScore = 40;
      else if (daysSinceLastAnswer <= 30) recencyScore = 20;
      else recencyScore = 0;
    }

    // Weighted mastery score
    const masteryScore = Math.round(
      correctRate * CORRECT_RATE_WEIGHT +
    ...
    const isAutoMastered =
      (masteryScore >= MASTERY_THRESHOLD && stats.totalAppeared >= MIN_APPEARANCES) ||
      (stats.consecutiveCorrect >= CONSECUTIVE_CORRECT_THRESHOLD && stats.totalAppeared >= 2);

    const updated = await prisma.wordMasteryStats.update({
      where: {
        userId_wordId: {
          userId,
          wordId,
  ...
  static async recalculateAll(userId: string) {
    const statsList = await prisma.wordMasteryStats.findMany({
      where: { userId },
    });
  ...
  static async getMasteryStats(userId: string, wordId: string) {
    const stats = await prisma.wordMasteryStats.findUnique({
      where: {
        userId_wordId: {
          userId,
  ...
  static async listMasteryStats(userId: string, options?: { minScore?: number; maxScore?: number; onlyAutoMastered?: boolean; limit?: number; offset?: number }) {
    ...
    if (options?.minScore !== undefined || options?.maxScore !== undefined) {
      where.masteryScore = {};
      if (options.minScore !== undefined) where.masteryScore.gte = options.minScore;
      if (options.maxScore !== undefined) where.masteryScore.lte = options.maxScore;
    }
  ...
  static async resetStats(userId: string, wordId?: string) {
    if (wordId) {
      await prisma.wordMasteryStats.deleteMany({
        where: { userId, wordId },
      });
      ...
      const autoMastered = await prisma.wordMasteryStats.findMany({
