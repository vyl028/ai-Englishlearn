...
// 智能修复截断的 JSON，处理嵌套结构
function trySmartFixTruncatedJson(text) {
    let fixed = text.trim();
...
function validateAndNormalizeEssayReview(raw) {
    ...
    if (Array.isArray(raw.issues)) {
        result.issues = raw.issues.map((issue) => {
            const normalized = {
                category: 'other',
                suggestion: '',
                explanationZh: '',
            ...
            if (issue && typeof issue === 'object') {
                ...
                if (cat && validCategories.includes(cat)) {
                    ...
                    if (catLower.includes('grammar'))
                        normalized.category = 'grammar';
                    else if (catLower.includes('spell'))
                        normalized.category = 'spelling';
                    else if (catLower.includes('tense'))
                        normalized.category = 'tense';
                    else if (catLower.includes('logic'))
                        normalized.category = 'logic';
                    else if (catLower.includes('coheren'))
                        normalized.category = 'coherence';
                    else if (catLower.includes('task'))
                        normalized.category = 'task_response';
                    else if (catLower.includes('word') || catLower.includes('vocab'))
                        normalized.category = 'word_choice';
                    else if (catLower.includes('punct'))
                        normalized.category = 'punctuation';
                    else if (catLower.includes('style'))
                        normalized.category = 'style';
                }
                // 处理 severity
                ...
                if (sev && validSeverities.includes(sev)) {
                    ...
                    if (sevLower.includes('high') || sevLower.includes('严重'))
                        normalized.severity = 'high';
                    else if (sevLower.includes('medium') || sevLower.includes('中等'))
                        normalized.severity = 'medium';
                    else if (sevLower.includes('low') || sevLower.includes('轻微'))
    ...
    if (Array.isArray(beforeAfterSource)) {
        result.beforeAfter = beforeAfterSource.map((item) => ({
...
注意：analysisZh, grammarZh, usageZh 这三个字段的内容必须是纯中文，不要用英文！`,
        ...
        const getChineseContent = (zhField, ...fallbackFields) => {
            // 优先检查 zh 结尾的字段
            if (containsChinese(zhField))
                return zhField;
            // 然后检查其他字段是否有中文内容
