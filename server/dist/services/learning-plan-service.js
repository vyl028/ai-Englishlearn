...
class LearningPlanService {
    // ===== Evaluation =====
    static async generateEvaluation(userId) {
        const now = new Date();
        const sevenDaysAgo = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
        const thirtyDaysAgo = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
        // --- Vocabulary dimension ---
        const allMastery = await database_1.default.wordMasteryStats.findMany({
            where: { userId },
            include: {
                word: {
        ...
        const atRiskWords = allMastery.filter((m) => {
            if (m.masteryScore >= MASTERY_AT_RISK_THRESHOLD)
                return false;
            if (!m.lastAnsweredAt)
                return true;
        ...
        const recentWords = await database_1.default.word.findMany({
            where: {
                userId,
        ...
        const vocabularyScore = Math.min(100, Math.round(masteryRate * 0.6 +
            Math.max(0, 100 - weakWords.length * 5) * 0.25 +
            Math.max(0, 100 - atRiskWords.length * 5) * 0.15));
        // --- Practice dimension ---
        const recentAnswers = await database_1.default.practiceAnswer.findMany({
            where: {
                practiceRecord: {
                    userId,
        ...
        const recentPracticeRecords = await database_1.default.practiceRecord.findMany({
        ...
        // --- Activity dimension ---
        const stats = await user_stats_service_1.UserStatsService.getLearningStats(userId);
        const goals = await user_stats_service_1.UserStatsService.getGrowthGoals(userId);
        ...
        const activityScore = Math.round(Math.min(100, streakDays * 10) * 0.3 +
            Math.min(100, weeklyActiveDays * 15) * 0.3 +
            Math.min(100, avgAchievementRate) * 0.4);
        // --- Overall ---
        const overallScore = Math.round(vocabularyScore * VOCABULARY_WEIGHT +
            practiceScore * PRACTICE_WEIGHT +
            activityScore * ACTIVITY_WEIGHT);
        // --- Trend (compare with last week's overall if available) ---
        const lastWeekPlan = await database_1.default.learningPlan.findFirst({
        ...
        if (lastWeekPlan) {
            try {
                ...
                if (overallScore > prev + 5)
                    trend = 'up';
                else if (overallScore < prev - 5)
                    trend = 'down';
            }
            catch { /* ignore */ }
        }
        ...
        if (weakWords.length >= 5)
            weakPoints.push(`薄弱词汇较多（${weakWords.length}个），建议加强复习`);
        if (atRiskWords.length >= 5)
            weakPoints.push(`遗忘风险词汇${atRiskWords.length}个，需要及时巩固`);
        if (overallCorrectRate < 60 && totalRecentAnswers > 0)
            weakPoints.push('练习题正确率偏低，建议回顾错题');
        if (weeklyPracticeCount < 2)
            weakPoints.push('本周练习次数较少，建议保持每日练习');
        if (avgAchievementRate < 50)
            weakPoints.push('本周目标达成率不足50%，可适当调整目标');
        if (masteryRate >= 70)
            strengths.push(`词汇掌握率达到${masteryRate}%`);
        if (overallCorrectRate >= 80)
            strengths.push('练习题正确率优秀');
        if (streakDays >= 7)
            strengths.push(`已连续学习${streakDays}天，保持得非常好`);
        if (avgAchievementRate >= 100)
            strengths.push('本周目标已达成');
        return {
            overallScore,
            trend,
            dimensions: {
    ...
    static async generateRecommendations(userId) {
        ...
        if (strongType)
            recommendedTypes.push(strongType.type);
        if (recommendedTypes.length < 2) {
            const allTypes = ['mcq', 'fill_blank', 'reorder'];
            for (const t of allTypes) {
                if (!recommendedTypes.includes(t))
                    recommendedTypes.push(t);
            }
        }
    ...
    static async generateDailyPlan(userId) {
        ...
        if (evaluation.dimensions.vocabulary.details.totalWords < 20 || weeklyWords < goals.weeklyWordsGoal * 0.5) {
            tasks.push({
                id: generateTaskId(),
                type: 'capture_words',
                title: '采集新单词',
        ...
        const title = estimatedMinutes <= 15
            ...
            : estimatedMinutes <= 30
                ...
                : '今日挑战计划';
        // Persist
        const plan = await database_1.default.learningPlan.upsert({
            where: {
    ...
    static async getTodayPlan(userId) {
        ...
        const plan = await database_1.default.learningPlan.findUnique({
            where: {
                userId_dateKey_planType: {
                    userId,
                    dateKey,
    ...
    static async getOrCreateTodayPlan(userId) {
        const existing = await this.getTodayPlan(userId);
        if (existing)
            return existing;
        return this.generateDailyPlan(userId);
    }
    static async updatePlanStatus(userId, planId, status) {
        const plan = await database_1.default.learningPlan.findFirst({
            where: { id: planId, userId },
        });
        if (!plan)
            throw new Error('PLAN_NOT_FOUND');
        const updated = await database_1.default.learningPlan.update({
            where: { id: planId },
            data: { status },
        });
        return {
    ...
    static async listPlanHistory(userId, limit = 30) {
        const plans = await database_1.default.learningPlan.findMany({
            where: { userId },
            orderBy: { createdAt: 'desc' },
            take: limit,
        });
