...
class UserStatsService {
    ...
    static async getLearningStats(userId) {
        const stats = await database_1.default.learningStats.findUnique({
            where: { userId },
        });
        if (!stats) {
            // Create default stats if not exists
            await database_1.default.learningStats.create({
                data: { userId },
            });
    ...
    static async updateLearningStats(userId, state) {
        await database_1.default.learningStats.upsert({
            where: { userId },
            create: {
                userId,
                totalXp: state.xp,
    ...
    static async recordLearningEvent(userId, event) {
    ...
    static addXp(state, dateKey, amount) {
    ...
    static addDayStat(state, dateKey, patch) {
        const day = state.daily[dateKey] || { xpEarned: 0, wordsAdded: 0, practiceCompleted: 0, storiesGenerated: 0 };
        return {
            ...state,
            daily: {
    ...
    // ========== GrowthGoals ==========
    static async getGrowthGoals(userId) {
        const goals = await database_1.default.growthGoals.findUnique({
            where: { userId },
        });
        if (!goals) {
            await database_1.default.growthGoals.create({
                data: { userId },
            });
    ...
    static async updateGrowthGoals(userId, goals) {
        await database_1.default.growthGoals.upsert({
            where: { userId },
            create: {
                userId,
    ...
    static async getReadingStats(userId, articleKey) {
        const stats = await database_1.default.readingQuestionStats.findUnique({
    ...
    static async updateReadingStats(userId, articleKey, score, total) {
        const now = new Date();
        const existing = await database_1.default.readingQuestionStats.findUnique({
            where: { userId_articleKey: { userId, articleKey } },
        });
        if (existing) {
            const newBest = Math.max(existing.best, score);
            await database_1.default.readingQuestionStats.update({
            ...
            await database_1.default.readingQuestionStats.create({
                data: {
                    userId,
                    articleKey,
                    attempts: 1,
    ...
    static async getSpeakingStats(userId, dateKey) {
        const stats = await database_1.default.speakingTrainingStats.findUnique({
    ...
    static async getAllSpeakingStats(userId) {
        const stats = await database_1.default.speakingTrainingStats.findMany({
            where: { userId },
            orderBy: { dateKey: 'asc' },
        });
        const days = {};
    ...
    static async recordSpeakingAttempt(userId, dateKey, score, at) {
        const timestamp = at?.getTime() || Date.now();
        const safeScore = Math.max(0, Math.min(100, Math.round(score)));
        const existing = await database_1.default.speakingTrainingStats.findUnique({
            where: { userId_dateKey: { userId, dateKey } },
        });
        if (existing) {
            await database_1.default.speakingTrainingStats.update({
                where: { id: existing.id },
                data: {
                    attempts: existing.attempts + 1,
            ...
            await database_1.default.speakingTrainingStats.create({
                data: {
                    userId,
                    dateKey,
                    attempts: 1,
    ...
    // ========== LearningEvents ==========
    static async getLearningEvents(userId, limit = 120) {
        const events = await database_1.default.learningEvent.findMany({
            where: { userId },
            orderBy: { at: 'desc' },
            take: limit,
        });
    ...
    static async addLearningEvent(userId, event) {
        const at = event.at || new Date();
        let data = {
            userId,
            type: event.type,
        ...
        await database_1.default.learningEvent.create({ data });
        // Also update gamification stats
        await this.recordLearningEvent(userId, event);
    }
    static async clearLearningEvents(userId) {
        await database_1.default.learningEvent.deleteMany({
            where: { userId },
        });
    }
    ...
    static async resetAllStats(userId) {
        await database_1.default.$transaction([
            database_1.default.learningStats.update({
                where: { userId },
                data: {
                    totalXp: 0,
                    streakDays: 0,
            ...
            database_1.default.readingQuestionStats.deleteMany({ where: { userId } }),
            database_1.default.speakingTrainingStats.deleteMany({ where: { userId } }),
            database_1.default.learningEvent.deleteMany({ where: { userId } }),
            // Don't delete growth goals, just reset to defaults
            database_1.default.growthGoals.update({
                where: { userId },
