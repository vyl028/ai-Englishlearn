...
### 新增/修改内容
- **跨设备适配 - 阶段二：任务1 摄像头采集流程优化**
  - `WordCaptureForm` Camera 功能移动端优化：
    - 视频预览区：移动端使用 `aspect-[4/3]` 纵向比例，更大取景区域
    - 摄像头切换按钮：改为悬浮在视频右上角，圆形设计（48×48px）
    - 添加摄像头类型提示（前置/后置），显示在视频左下角
    - 拍照按钮：改为大圆形设计（64×64px），更醒目的拍照体验
    - 优化整体布局，更符合原生相机应用体验
...
### 背景/原因
- 跨设备适配阶段二：优化核心功能在移动端的交互体验，使摄像头采集更符合移动端用户习惯

### 如何验证
- 运行：`npm run typecheck`
- 使用 Chrome DevTools 设备模拟器（375px宽度）测试：
  - 切换至"拍照"标签，视频预览区域正常显示
  - 摄像头切换按钮悬浮在视频右上角（设备支持时显示）
  - 拍照按钮为圆形大按钮，点击可拍照
  - 整体布局类似原生相机应用
